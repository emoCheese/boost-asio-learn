...
MsgNode::MsgNode(short max_len, short cur_len)
	: _cur_len(cur_len), _total_len(max_len)
{
	_data = new char[max_len + 1]();
	_data[max_len] = 0;
}
...
RecvNode::RecvNode(short total_len, short cur_len, short msg_id)
	: MsgNode(total_len, cur_len), _msg_id(msg_id)
{
