...
void CServer::start_accept()
{
	...
	_acceptor.async_accept(new_session->GetSocket(), 
		std::bind(&CServer::handler_accept, this, new_session, std::placeholders::_1)
	);
}

void CServer::handler_accept(std::shared_ptr<CSession> new_session, const boost::system::error_code& error)
{
	if (!error) {
		new_session->Start();
